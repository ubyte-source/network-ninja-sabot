...
ENV STARTUP_COMMAND_RUN_TFTP="/usr/bin/sudo /usr/sbin/in.tftpd --ipv4 --foreground --user sabot --address :69 --secure /var/tftpboot --blocksize 1380 --port-range 40000:48000 --timeout 172800 | in.tftpd"
ENV STARTUP_COMMAND_RUN_NODERED="/usr/bin/node /usr/local/lib/node_modules/node-red/red.js --userDir /data | node-red"

ARG TIMEZONE="UTC"

RUN apk update && \
    apk add --no-cache sudo nmap sqlite iputils nodejs npm tftp-hpa git openssh && \
